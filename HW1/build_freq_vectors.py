...
def compute_cooccurrence_matrix(corpus, vocab):
	...
	#Co-ocurrence matrix
	C = np.zeros((vocab.size, vocab.size))

	for row in corpus:
		context = vocab.text2idx(row)
		for center in context:
			for word in context:
				C[center, word] += 1
...
def compute_ppmi_matrix(corpus, vocab):
	...
	C = compute_cooccurrence_matrix(corpus, vocab) + 1e-5
	...
	for idx, _ in np.ndenumerate(C):
		...
		PPMI[i, j] = max(1e-5, np.log((C[i, j] * N) / (C[i, i] * C[j, j])))
...
def main_freq():
	...
	dataset = load_from_disk("/home/magraz/AI539_NLP/HW1/data/ag_news")
...
def plot_word_vectors_tsne(word_vectors, vocab):
	...
	ax3.plot(coords[top_word_idx,0], coords[top_word_idx,1], 'o', markerfacecolor='none', markeredgecolor='k', alpha=0.5, markersize=3)

	for i in tqdm(top_word_idx):
		ax3.annotate(vocab.idx2text([i])[0],
